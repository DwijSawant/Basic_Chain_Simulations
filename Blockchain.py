...
class Block:
    def __init__(self, index, timestamp, data, prevhash, nonce=0):
        ...
        self.nonce = nonce
        self.hash = self.calculate_hash()  # Hash based only on nonce
...
while i<=numberofblocks :
    ...
    i += 1
#Here will insert basic if else loop along with will try to hinder with hash to show how hashes effect the chain
...
if Manipulation == "y" :
    ...
    blockchain[block_num].hash = blockchain[block_num].calculate_hash()
